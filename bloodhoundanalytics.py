...
class FrontPage(object):
    ...
    def create_qa_statistics(self, sheet):
        ...
        query = """MATCH (u:User {domain:$domain})
                    ...
                    RETURN toInteger(100.0 * COUNT(u) / userCount)
                    """
        ...
        query = """MATCH (c:Computer {domain:$domain})
                    ...
                    RETURN toInteger(100.0 * COUNT(c) / ComputerCount)
                    """
        ...
        self.write_single_cell(
            sheet, 7, 3, "Guest Account: {}".format(guest_accounts))
...
class MainMenu(cmd.Cmd):
    ...
    def validate_domain(self):
        ...
        for result in session.run("MATCH (n {domain:$domain}) RETURN COUNT(n)", domain=self.domain):
            if (int(result[0]) > 0):
                print("Domain {} validated!".format(self.domain))
    ...
    def create_workbook(self):
        wb = Workbook()
        ws = wb.active
        ws.title = '{} Overview'.format(self.domain)
